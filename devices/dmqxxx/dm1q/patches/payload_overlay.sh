#!/bin/bash
# Device payload overlay for dm1q.
# Place files under: devices/dmqxxx/dm1q/patches/payload/<partition>/...
...
payload_overlay() {
  ...
  if [[ ! -d "$payload_root" ]]; then
    log_verbose "[Payload] No dm1q payload for partition: $partition_name"
    return 0
  fi
  ...
  log_info "[Payload] Applying dm1q payload: $payload_root -> $target_root"
  ...
  log_info "[Payload] dm1q payload applied for $partition_name"
}

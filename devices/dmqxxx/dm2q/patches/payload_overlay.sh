#!/bin/bash
# Device payload overlay for dm2q.
# Place files under: devices/dmqxxx/dm2q/patches/payload/<partition>/...
...
payload_overlay() {
  ...
  if [[ ! -d "$payload_root" ]]; then
    log_verbose "[Payload] No dm2q payload for partition: $partition_name"
    return 0
  fi
  ...
  log_info "[Payload] Applying dm2q payload: $payload_root -> $target_root"
  ...
  log_info "[Payload] dm2q payload applied for $partition_name"
}

#!/bin/bash
# Shared payload overlay for dmqxxx family.
# Place files under: devices/dmqxxx/patches/payload/<partition>/...
# Example: payload/system/priv-app/MyApp/MyApp.apk
...
payload_overlay() {
  ...
  if [[ ! -d "$payload_root" ]]; then
    log_verbose "[Payload] No shared payload for partition: $partition_name"
    return 0
  fi
  ...
  log_info "[Payload] Applying shared payload: $payload_root -> $target_root"
  ...
  log_info "[Payload] Shared payload applied for $partition_name"
}
